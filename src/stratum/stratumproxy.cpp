    ...
    while (!m_interrupt.load() && m_sockfd >= 0) {
        ...
        if (n <= 0) {
            if (n == 0) {
                ...
                RecordError(strprintf("recv() error: %s", strerror(errno)));
            }
            break;
        }
...
void StratumProxyConn::HandleNotification(const StratumNotification &notif) {
    if (notif.method == "mining.notify") {
        ...
        if (m_callbacks.onNotify) {
            std::string raw = SerializeNotify(notif.method, notif.params);
...
bool StratumProxyConn::DoSubscribe() {
    ...
    params.push_back("lotus-stratum/1.0");
    ...
    for (const auto &line : lines) {
        ...
        auto *resp = std::get_if<StratumResponse>(&msg);
        if (resp && resp->id == 1) {
            if (resp->result.isArray() && resp->result.size() >= 3) {
                m_upstreamExtranonce1 = resp->result[1].get_str();
                m_upstreamExtranonce2Size = resp->result[2].get_int();
                LogPrint(BCLog::MINING,
                         "Stratum proxy: subscribed to %s, en1=%s en2size=%d\n",
                         GetLabel(), m_upstreamExtranonce1,
                         m_upstreamExtranonce2Size);
                return true;
            }
        }
        auto *notif = std::get_if<StratumNotification>(&msg);
        if (notif) {
