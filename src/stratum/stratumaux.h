...
struct AuxWorkTemplate {
    uint256 auxBlockHash;   // hash of the Lotus header (block identity)
    uint32_t nChainId;      // AUXPOW_CHAIN_ID for Lotus
...
struct AuxPowChainProof {
    ...
    CParentBlockHeader parentHeader;
};

class StratumAuxManager {
public:
    StratumAuxManager(CChainState &chainstate, const CTxMemPool *mempool,
     ...
     */
    bool CreateAuxWork(AuxWorkTemplate &out, std::string &error);
     ...
     */
    bool ValidateParentPow(const CParentBlockHeader &parentHeader,
                           uint32_t dogeNBits,
